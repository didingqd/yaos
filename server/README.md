...
- `enable_r2 = true`（默认）时：
  - 自动检查并创建（若不存在）桶，桶名优先读取 GitHub Secret `R2_BUCKET_NAME`，未配置时回退 `yaos-bucket`
...
- `CF_ACCOUNT_ID`
- `CF_API_TOKEN`
- `R2_BUCKET_NAME`（可选，仅在你想覆盖默认桶名时配置）

> 修改点：新增 Actions 一键部署说明，支持 R2 可选且默认检查并创建；R2 桶名支持 Secret 覆盖并带默认回退。
